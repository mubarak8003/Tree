...
export const SupportTab: React.FC<SupportTabProps> = ({
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {/* 1. Header KPI & Action Bar (Hidden on mobile when in CHAT view so chat has full vertical space) */}
      <div className={`grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 ${
        mobileViewMode === "CHAT" ? "hidden lg:grid" : "grid"
      }`}>
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-5 ${
        mobileViewMode === "CHAT"
          ? "h-[calc(100dvh-6.5rem)] min-h-[480px] max-h-[920px]"
          : "h-[calc(100dvh-11rem)] min-h-[500px] max-h-[920px]"
      } lg:h-[calc(100vh-14rem)] lg:min-h-[620px]`}>
        
        {/* Left Column: Tickets Master List (Visible when mobileViewMode is LIST, or on desktop lg:flex) */}
        <div className={`lg:col-span-4 flex flex-col min-h-0 bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden ${
        ...
        <div className={`lg:col-span-8 flex flex-col min-h-0 bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden ${
          ...
          {activeTicket ? (
            <>
              ...
              <div 
                ...
                className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-4 bg-slate-100/40 dark:bg-slate-950/20"
              >
              ...
              <form 
                ...
                className="p-3 sm:p-4 pb-5 sm:pb-4 border-t border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 space-y-2.5 z-10"
              >
                <div className="relative flex items-center">
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl text-xs sm:text-sm font-medium text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all resize-none max-h-28 min-h-[56px]"
                  />
                </div>

                <div className="flex items-center justify-between gap-2">
                  <label className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 cursor-pointer select-none truncate">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="truncate">Auto-resolve on reply</span>
                  </label>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="px-4 sm:px-5 py-2 sm:py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-40 text-white text-xs sm:text-sm font-black rounded-xl shadow-md transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer whitespace-nowrap active:scale-98"
                    >
                      {isSendingReply ? (
                        <>
